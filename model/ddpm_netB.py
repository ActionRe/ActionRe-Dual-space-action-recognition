...
class unit_gcn(nn.Module):  # spatial modeling
...
class Modelys(nn.Module):
    def __init__(self, num_class=60, num_point=25, num_person=1, graph=None, graph_args=dict(), in_channels=3,
                 drop_out=0, adaptive=True):
        global base_channel, base_channel
        super(Modelys, self).__init__()

        if graph is None:
            raise ValueError()
        else:
            Graph = import_class(graph)
            self.graph = Graph(**graph_args)  # graph

        A = self.graph.A  # 3,25,25

        self.num_class = num_class
        self.num_point = num_point
        self.numperson = num_person
        self.data_bn = nn.BatchNorm1d(num_person * in_channels * num_point)
        #self.data_bn = nn.BatchNorm1d(60)
